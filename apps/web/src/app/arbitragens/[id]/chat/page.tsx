    ...
    <div className="flex flex-col h-[calc(100vh-0px)]">
      ...
      <div className="bg-white border-b px-6 py-3 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <Link href={`/arbitragens/${id}`} className="text-primary-600 hover:underline text-sm">
            &larr;
          </Link>
          <h1 className="font-semibold text-gray-800">Chat do Caso</h1>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
        {messages.length === 0 && (
          <p className="text-center text-gray-400 py-8">Nenhuma mensagem. Inicie a conversa.</p>
        )}
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] rounded-xl px-4 py-2 ${
                isMe ? 'bg-primary-600 text-white' : 'bg-white shadow'
              }`}>
                {!isMe && (
      ...
      <div className="bg-white border-t p-4">
        <div className="flex gap-2 max-w-4xl mx-auto">
          <textarea
            ...
            className="flex-1 px-4 py-2 border border-gray-300 rounded-xl resize-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          />
          <button
            onClick={handleSend}
